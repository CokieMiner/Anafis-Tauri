...
import SequenceChart from '@/shared/dataLibrary/components/SequenceChart';
...
const SequenceDetails: React.FC<SequenceDetailsProps> = memo(
  ({ sequence, statistics, onPin, onEdit, onDuplicate, onDelete }) => {
    if (!sequence) {
      return (
        <Box
          sx={{
          ...
          <Table stickyHeader size="small">
            ...
            <TableBody>
              {sequence.data.slice(0, 50).map((value, index) => (
                // biome-ignore lint/suspicious/noArrayIndexKey: Order of sequence data is static
                <TableRow key={`series-val-${index}`}>
                  <TableCell>{index}</TableCell>
                  <TableCell>{value}</TableCell>
                  {sequence.uncertainties && (
                    <TableCell>
                      {sequence.uncertainties[index] ?? '-'}
                    </TableCell>
                  )}
                </TableRow>
              ))}
              {sequence.data.length > 50 && (
                <TableRow>
                  <TableCell
                    colSpan={sequence.uncertainties ? 3 : 2}
                    align="center"
                  >
                    <Typography variant="caption" color="text.secondary">
