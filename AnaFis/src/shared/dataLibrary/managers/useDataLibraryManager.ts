...
export const useDataLibrary = () => {
  ...
  const [selectedSequence, setSelectedSequence] = useState<DataSequence | null>(
    null
  );
  const [selectedStats, setSelectedStats] = useState<SequenceStatistics | null>(
    null
  );
  ...
  const [sortBy, setSortBy] = useState<
    'name' | 'date_created' | 'date_modified' | 'size'
  >('date_modified');
  ...
  const loadSequences = useCallback(async () => {
    ...
    try {
      const response = await invoke<SequenceListResponse | ErrorResponse>(
        'get_sequences',
  ...
  const loadStatistics = useCallback(async (id: string) => {
    try {
      const response = await invoke<SequenceStatistics | null | ErrorResponse>(
        'get_sequence_stats',
        { id }
      );
      if (isErrorResponse(response)) {
      ...
      setError(
        `Failed to load statistics: ${err instanceof Error ? err.message : String(err)}`
      );
    }
  }, []);
  ...
  const handlePinSequence = useCallback(
    async (id: string, isPinned: boolean) => {
      try {
        const response = await invoke<undefined | ErrorResponse>(
          'pin_sequence',
          {
            id,
            isPinned: !isPinned,
        ...
        setError(
          `Failed to pin sequence: ${err instanceof Error ? err.message : String(err)}`
        );
      }
    },
  ...
  const handleDeleteSequence = useCallback(
    async (id: string) => {
      try {
        ...
        setError(
          `Failed to delete sequence: ${err instanceof Error ? err.message : String(err)}`
        );
      }
    },
    [selectedSequence, loadSequences]
    ...
    async (ids: Set<string>) => {
      ...
      try {
        ...
        const deleteResults = await Promise.allSettled(
          idsArray.map((id) =>
            invoke<undefined | ErrorResponse>('delete_sequence', { id })
          )
        );
        ...
        setError(
          `Unexpected error during bulk delete: ${err instanceof Error ? err.message : String(err)}`
        );
  ...
  const handleDuplicateSequence = useCallback(
    async (id: string, name: string) => {
      try {
        ...
        const response = await invoke<string | ErrorResponse>(
          'duplicate_sequence',
          { id, newName }
        );
        if (isErrorResponse(response)) {
        ...
        setError(
          `Failed to duplicate sequence: ${err instanceof Error ? err.message : String(err)}`
        );
      }
    },
    [loadSequences]
  ...
  const handleUpdateSequence = useCallback(
    async (request: UpdateSequenceRequest) => {
      try {
        const response = await invoke<undefined | ErrorResponse>(
          'update_sequence',
          {
        ...
        if (selectedSequence?.id === request.id) {
          ...
          if (request.name !== undefined) {
            updatedSequence.name = request.name;
          }
          if (request.description !== undefined) {
            updatedSequence.description = request.description;
          }
          if (request.unit !== undefined) {
            updatedSequence.unit = request.unit;
          }
          if (request.tags !== undefined) {
            updatedSequence.tags = request.tags;
          }
  ...
  const handleSetSortBy = useCallback(
    (value: 'name' | 'date_created' | 'date_modified' | 'size') => {
      setCurrentPage(0);
      setSortBy(value);
    },
    []
