...
export const AppToolbar = React.memo<AppToolbarProps>(({ onAddTab, createTabContent }) => {
  ...
  const handleProjectMenuClick = useCallback((event: React.MouseEvent<HTMLButtonElement>) => {
  ...
  const asyncHandlers = useMemo(() => ({
    openDataLibrary: async () => {
      try {
        ...
        console.error('[AppToolbar] Failed to open data library window:', error);
      }
    },
    openUncertaintyCalculator: async () => {
      try {
        ...
        console.error('[AppToolbar] Failed to open uncertainty calculator window:', error);
      }
    },
    openSettings: async () => {
      try {
        ...
        console.error('[AppToolbar] Failed to open settings window:', error);
      }
    },
  }), []);
  ...
  const tabButtonHandlers = useMemo(() => 
    TAB_BUTTONS_CONFIG.reduce((acc, config) => {
      acc[config.type] = () => {
        const tabId = `${config.type}-${++tabIdCounter.current}`;
        onAddTab(tabId, config.label, createTabContent(config.type, tabId));
      };
      return acc;
  ...
  return (
    <AppBar position="static" sx={TOOLBAR_STYLES}>
      <Toolbar sx={{ gap: 1 }}>
        ...
        >
          <MenuItem onClick={handleProjectMenuClose} sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
        ...
        {TAB_BUTTONS_CONFIG.map((config) => {
          ...
          if (!handler) {return null;} // Skip if handler doesn't exist
