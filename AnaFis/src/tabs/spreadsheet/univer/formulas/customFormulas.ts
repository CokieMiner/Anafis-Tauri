...
export function registerCustomFunctions(
  formulaEngine: IRegisterFunctionService
) {
  // Register SINC function with high precision using JavaScript
  formulaEngine.registerFunction({
    name: 'SINC',
    func: (
      ...args: FormulaFunctionValueType[]
    ): FormulaFunctionResultValueType => {
      const x =
        typeof args[0] === 'number'
          ? args[0]
          : parseFloat(args[0] as string) || 0;
      if (x === 0) {
        return 1;
      }
      return Math.sin(x) / x;
  ...
  formulaEngine.registerFunction({
    ...
    description:
      'Inverse hyperbolic cotangent function: acoth(x) = 0.5 * ln((x+1)/(x-1))',
  });
  ...
  formulaEngine.registerFunction({
    ...
    description:
      'Inverse hyperbolic secant function: asech(x) = ln((1 + √(1-x²))/x)',
  });
