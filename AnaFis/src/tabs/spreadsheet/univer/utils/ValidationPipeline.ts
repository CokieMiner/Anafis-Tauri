...
export const VALIDATION_ERRORS = {
  ...
  MISSING_VARIABLES: {
    ...
    message: 'At least one variable is required to perform uncertainty propagation.'
  },
  MISSING_VALUE_RANGE: {
  ...
  MISSING_OUTPUT_VALUE_RANGE: {
    ...
    message: 'Output value range is required. Specify where to write the calculated values (e.g., "C1:C10").'
  },
  MISSING_OUTPUT_UNCERTAINTY_RANGE: {
    code: 'MISSING_OUTPUT_UNCERTAINTY_RANGE',
    message: 'Output uncertainty range is required. Specify where to write the calculated uncertainties (e.g., "D1:D10").'
  },
  DUPLICATE_VARIABLE_NAMES: {
    code: 'DUPLICATE_VARIABLE_NAMES',
    message: 'Duplicate variable names found: {duplicates}. Each variable must have a unique name.'
  },
  INVALID_CONFIDENCE_LEVEL: {
    code: 'INVALID_CONFIDENCE_LEVEL',
    message: 'Variable "{name}" confidence level ({confidence}%) must be between 50% and 99.9%.'
  },

  // Range validation
  INVALID_RANGE_FORMAT: {
    code: 'INVALID_RANGE_FORMAT',
    message: 'Range "{range}" is not valid A1 notation. Use format like "A1:A10" (column letter + row number).'
  },
  RANGES_OVERLAP: {
    code: 'RANGES_OVERLAP',
    message: 'Output ranges cannot overlap. Value range "{valueRange}" and uncertainty range "{uncertaintyRange}" overlap. Choose different columns or adjust the ranges.'
  },
  RANGE_OVERLAPS_INPUT: {
    code: 'RANGE_OVERLAPS_INPUT',
    message: 'Output ranges cannot overlap with input data ranges. Please choose different columns for your results that don\'t conflict with your input data.'
  },
  RANGE_NOT_ACCESSIBLE: {
    code: 'RANGE_NOT_ACCESSIBLE',
    message: 'Range "{range}" is not accessible: {details}. Make sure the range exists and is not protected or read-only.'
  },
  RANGE_NOT_WRITABLE: {
    code: 'RANGE_NOT_WRITABLE',
    message: 'Cannot write to range "{range}": {details}. Make sure the range is not protected or read-only.'
  },

  // Data validation
  NON_NUMERIC_DATA: {
    code: 'NON_NUMERIC_DATA',
    message: 'Variable "{name}" {type} range "{range}" contains non-numeric data. All values must be numbers for uncertainty propagation.'
  },
  EMPTY_RANGE: {
    code: 'EMPTY_RANGE',
    message: 'Variable "{name}" {type} range "{range}" is empty. Please provide numeric data.'
  },
  DIMENSION_MISMATCH: {
    code: 'DIMENSION_MISMATCH',
    message: 'Variable "{name}": uncertainty range "{uncertaintyRange}" ({uncertaintyRows} rows) doesn\'t match value range "{valueRange}" ({valueRows} rows). Both ranges must have the same number of data points.'
  },

  // General validation
  SPREADSHEET_NOT_READY: {
    code: 'SPREADSHEET_NOT_READY',
    message: 'Spreadsheet is not ready. Please wait for the spreadsheet to fully load before running uncertainty propagation.'
  },
  NO_INPUT_RANGES: {
    code: 'NO_INPUT_RANGES',
    message: 'No input ranges found. Make sure at least one variable has a value range specified.'
  }
} as const;
...
export const VALIDATION_WARNINGS = {
  NO_UNCERTAINTY_RANGES: {
    ...
    message: 'Some variables have no uncertainty ranges - they will be treated as having zero uncertainty.'
  }
} as const;
...
export class BasicFieldValidator implements Validator {
  validate(context: ValidationContext): Promise<ValidationResult> {
    ...
    if (missingValueRanges.length > 0) {
      ...
      errors.push({
        ...
        message: VALIDATION_ERRORS.MISSING_VALUE_RANGE.message.replace('{name}', names)
      });
    }
    ...
    if (duplicates.length > 0) {
      errors.push({
        ...
        message: VALIDATION_ERRORS.DUPLICATE_VARIABLE_NAMES.message.replace('{duplicates}', duplicates.join(', '))
      });
    }
...
export class RangeFormatValidator implements Validator {
  validate(context: ValidationContext): Promise<ValidationResult> {
    const errors: ValidationError[] = [];
    ...
    try {
      ...
      errors.push({
        ...
        message: VALIDATION_ERRORS.INVALID_RANGE_FORMAT.message.replace('{range}', outputRanges.value)
      });
    }
    ...
    try {
      ...
      errors.push({
        ...VALIDATION_ERRORS.INVALID_RANGE_FORMAT,
        message: VALIDATION_ERRORS.INVALID_RANGE_FORMAT.message.replace('{range}', outputRanges.uncertainty)
      });
    }
...
export class OverlapValidator implements Validator {
  validate(context: ValidationContext): Promise<ValidationResult> {
    const errors: ValidationError[] = [];
    const { variables, outputRanges } = context;
    ...
    try {
      RangeValidator.validateNoOverlap(outputRanges.value, outputRanges.uncertainty);
    } catch (_error) {
      errors.push({
...
export class AccessibilityValidator implements Validator {
  async validate(context: ValidationContext): Promise<ValidationResult> {
    ...
    const validationPromises = [
      RangeValidator.validateAccessible(outputRanges.value, spreadsheet)
        .catch((_error: unknown) => ({
          type: 'accessible' as const,
          range: outputRanges.value,
          error: String(_error)
        })),
      RangeValidator.validateAccessible(outputRanges.uncertainty, spreadsheet)
        .catch((_error: unknown) => ({
          type: 'accessible' as const,
          range: outputRanges.uncertainty,
          error: String(_error)
        })),
      RangeValidator.validateWritable(outputRanges.value, spreadsheet)
        .catch((_error: unknown) => ({
          type: 'writable' as const,
          range: outputRanges.value,
          error: String(_error)
        })),
      RangeValidator.validateWritable(outputRanges.uncertainty, spreadsheet)
        .catch((_error: unknown) => ({
          type: 'writable' as const,
          range: outputRanges.uncertainty,
          error: String(_error)
    ...
    for (const result of results) {
      if (result.status === 'rejected' && result.reason) {
        const reason = result.reason as { type: 'accessible' | 'writable'; range: string; error: string };
...
export class DataValidator implements Validator {
  async validate(context: ValidationContext): Promise<ValidationResult> {
    ...
    for (const variable of variables) {
      if (variable.valueRange) {
        rangeRequests.push({ variable, type: 'value', range: variable.valueRange });
      }
      if (variable.uncertaintyRange) {
        rangeRequests.push({ variable, type: 'uncertainty', range: variable.uncertaintyRange });
      }
    }
    ...
    const resultsByVariable = new Map<string, {
    ...
    for (const [varName, results] of resultsByVariable) {
      ...
      // Validate value data
      if (results.value) {
        this.validateNumericData(variable, 'value', results.value.data, results.value.range, errors);
      }

      // Validate uncertainty data and dimension matching
      if (results.uncertainty && results.value) {
        this.validateNumericData(variable, 'uncertainty', results.uncertainty.data, results.uncertainty.range, errors);
        this.validateDimensions(variable, results.value, results.uncertainty, errors);
      }
    }
    ...
    if (valueResult.data.length !== uncertaintyResult.data.length) {
      errors.push({
        ...
        message: VALIDATION_ERRORS.DIMENSION_MISMATCH.message
          ...
          .replace('{uncertaintyRows}', uncertaintyResult.data.length.toString())
      });
    }
  }
}
...
export class ValidationPipeline {
    ...
    for (const validator of this.validators) {
      try {
        ...
        if (!result.isValid && result.errors.length > 0) {
          ...
          if (result.errors.some(e => ['MISSING_VARIABLES', 'MISSING_VALUE_RANGE', 'MISSING_OUTPUT_VALUE_RANGE', 'MISSING_OUTPUT_UNCERTAINTY_RANGE'].includes(e.code))) {
            return {
              isValid: false,
   ...
   */
  static setValidators(validators: Validator[]): void {
