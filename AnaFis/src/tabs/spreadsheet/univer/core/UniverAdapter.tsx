 ...
 */
const convertCellValueToFacade = (value: CellValue): { v?: string | number; f?: string } => {
...
const UniverAdapterInner = forwardRef<SpreadsheetRef, SpreadsheetProps>(
  ({ initialData, onCellChange, onFormulaIntercept, onSelectionChange, onReady, tabId }, ref) => {
    ...
    const memoizedOperations = useMemo(() => ({
      ...
      getCellValue: (cellRef: string): Promise<string | number | null> => {
        return Promise.resolve(getCellValue({ current: univerAPIRef.current }, cellRef));
      },
      ...
      getSelection: (): Promise<string | null> => {
        return Promise.resolve(getSelection({ current: univerAPIRef.current }));
      },
        ...
        return operationQueueRef.current.enqueue(() => {
          ...
          return safeSpreadsheetOperation(() => {
            ...
            if (rangeRef.includes(':')) {
              ...
              throw new Error(`Invalid range reference: ${rangeRef}. Use A1 notation for cells, column letters for columns, or row numbers for rows.`);
            }
      ...
      createSheet: (name: string, rows = DEFAULT_SHEET_ROWS, cols = DEFAULT_SHEET_COLS) => {
        if (!univerAPIRef.current) {
        ...
        const uniqueName = (() => {
          ...
          const existingNames = new Set(existingSheets.map(s => getSheetNameSafely(s, existingSheets)));
      ...
      deleteSheet: async (sheetId: string) => {
        ...
        try {
          const { ICommandService, IUniverInstanceService } = await import('@univerjs/core');
          ...
          const success = await commandService.executeCommand(RemoveSheetMutation.id, {
            unitId,
            subUnitId: sheetId,
          });
