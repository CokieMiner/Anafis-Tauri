...
export const useImportValidation = () => {
  ...
  const validateFileRange = useCallback(
    (
      ...
      if (
        (1 < selectedRows && selectedRows < fileMetadata.rowCount) ||
        (1 < selectedColumns && selectedColumns < fileMetadata.columnCount)
      ) {
        willTruncate = true;
        warnings.push(
          `Selected range (${selectedRows}×${selectedColumns}) is smaller than file data (${fileMetadata.rowCount}×${fileMetadata.columnCount}). Data will be truncated to fit.`
        );
      }
    ...
    (
      ...
      if (selectedRows < sequenceRows || selectedColumns < sequenceColumns) {
        if (!(selectedRows === 1 && selectedColumns === 1)) {
          willTruncate = true;
          warnings.push(
            `Selected range (${selectedRows}×${selectedColumns}) is smaller than data (${sequenceRows}×1). Data will be truncated.`
          );
        }
      }
    ...
    (
      ...
      if (selectedRows < sequenceRows || selectedColumns < sequenceColumns) {
        if (!(selectedRows === 1 && selectedColumns === 1)) {
          willTruncate = true;
          warnings.push(
            `Selected range (${selectedRows}×${selectedColumns}) is smaller than uncertainties (${sequenceRows}×1). Data will be truncated.`
          );
        }
      }
