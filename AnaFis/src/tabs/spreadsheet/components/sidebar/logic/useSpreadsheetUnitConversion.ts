...
import {
  ...
  parseRange,
  rangesOverlap,
} from '@/tabs/spreadsheet/components/sidebar/utils/rangeUtils';
import type {
  CellValue,
  SpreadsheetRef,
 ...
 */
export const useSpreadsheetUnitConversion = ({
  spreadsheetRef,
  fromUnit,
  toUnit,
  value,
    ...
    try {
      ...
      if (!compatible) {
        setCompatibilityError(
          `Units ${fromUnit} and ${toUnit} are not compatible`
        );
      } else {
        setCompatibilityError('');
      }
  ...
  const handleSidebarDisplay = useCallback(
    async (input: string, inputType: string) => {
      let numValue: number;
      ...
      const prefix = isConversionRate
        ? `Conversion rate: 1 ${fromUnit} = `
        : '';
  ...
  const handleInPlaceReplacement = useCallback(
    async (cellRef: string) => {
      if (!spreadsheetRef?.current) {
        throw new Error('Spreadsheet not available');
      }
      ...
      setResult(
        `Replaced ${cellRef}: ${numValue} ${fromUnit} → ${convertedValue.toFixed(6)} ${toUnit} (factor: ${conversionFactor.toFixed(6)})`
      );
    },
  ...
  const handleFillOutput = useCallback(
    async (
      fromInput: string,
      toInput: string,
      fromType: string,
      ...
      if (toType === 'cell') {
        // Single cell output
        await spreadsheetRef.current.updateCell(toInput, {
          v: conversionResult.value,
        });
      } else {
        // toType === 'range'
        ...
        const values: CellValue[][] = Array.from(
          { length: rangeFormat.rows },
          () =>
            Array.from({ length: rangeFormat.cols }, () => ({
              v: conversionResult.value,
            }))
        );
      ...
      setResult(
        `Filled ${toType === 'cell' ? '1 cell' : `${getRangeFormat(toInput).rows * getRangeFormat(toInput).cols} cells`} with ${conversionResult.value.toFixed(6)} ${toUnit}`
      );
    },
    [fromUnit, toUnit, spreadsheetRef]
  );
  ...
  const handleSameRangeConversion = useCallback(
    async (range: string) => {
      ...
      const convertedCount = convertedData
        .flat()
        .filter(
          (cell) => typeof cell.v === 'number' && !Number.isNaN(cell.v)
        ).length;

      setResult(
        `Applied conversion factor ${conversionFactor.toFixed(6)} to ${convertedCount}/${totalCells} cells in ${range}`
      );
    },
    [fromUnit, toUnit, spreadsheetRef]
  );
  ...
  const handleRangeToRange = useCallback(
    async (fromRange: string, toRange: string) => {
      ...
      if (fromRange !== toRange && rangesOverlap(fromRange, toRange)) {
        throw new Error(
          'Overlapping ranges not supported. Use same range for in-place conversion.'
        );
      }
      ...
      if (
        fromFormat.rows !== toFormat.rows ||
        fromFormat.cols !== toFormat.cols
      ) {
        throw new Error(
          `Range formats must match: ${fromFormat.rows}x${fromFormat.cols} vs ${toFormat.rows}x${toFormat.cols}`
        );
      }
      ...
      if (cellMapping.length === targetFormat.rows * targetFormat.cols) {
        ...
        const rangeValues: CellValue[][] = Array.from(
          { length: targetFormat.rows },
        ...
        for (let i = 0; i < cellMapping.length; i++) {
          ...
          const conversionResult = conversionResults[i];
          if (conversionResult !== undefined) {
            await spreadsheetRef.current.updateCell(mapping.toCell, {
              v: conversionResult.value,
            });
          }
        }
      ...
      setResult(
        `Converted ${values.length} values from ${fromRange} to ${toRange}`
      );
    },
  ...
  const applyConversionLogic = useCallback(
    async (
      fromInput: string,
      toInput: string,
      fromType: string,
    ...
    [
      ...
      handleFillOutput,
      handleRangeToRange,
      handleInPlaceReplacement,
    ]
  );

  // Main convert function
  const convert = useCallback(
    async (outputTarget: string = '') => {
      setError('');
      setResult('');
      ...
      if (spreadsheetRef?.current && !spreadsheetRef.current.isReady()) {
        setError(
          'Spreadsheet is not ready for cell operations. Please wait a moment and try again.'
        );
        return;
      }
