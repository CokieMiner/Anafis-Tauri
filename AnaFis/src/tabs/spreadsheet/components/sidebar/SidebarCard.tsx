...
const SidebarCard = React.memo<SidebarCardProps>(
  ({
    title,
    subtitle,
    icon,
    ...
    return (
      <Paper sx={combinedStyles} elevation={0}>
        {/* Header */}
        {(title ?? icon) && (
          <Box
            sx={headerBoxStyles}
            onClick={
              collapsible ? () => setExpanded((prev) => !prev) : undefined
            }
            onKeyDown={
              collapsible
                ? (event) => {
          ...
          >
            <Box sx={ICON_BOX_STYLES}>
              {icon && <Box sx={ICON_STYLES}>{icon}</Box>}
              {title && (
