...
const SpreadsheetContent: React.FC<SpreadsheetTabProps> = ({ tabId }) => {
  ...
  const isDetachedWindow = new URLSearchParams(window.location.search).has(
    'tabId'
  );

  // Workbook data context for proper synchronization
  const { getPendingWorkbookData, clearPendingWorkbookData } =
    useWorkbookData();
        ...
        <Box sx={{ display: 'flex', flex: 1, overflow: 'hidden', gap: 1 }}>
          ...
          >
            ...
            >
              ...
              {sidebarState.activeSidebar === 'export' &&
                sidebarState.services.exportService && (
                  <SidebarErrorBoundary
                    sidebarName="Export"
                    onClose={() => sidebarActions.setActiveSidebar(null)}
                  >
              ...
              {sidebarState.activeSidebar === 'import' &&
                sidebarState.services.importService && (
                  <SidebarErrorBoundary
                    sidebarName="Import"
                    onClose={() => sidebarActions.setActiveSidebar(null)}
                  >
