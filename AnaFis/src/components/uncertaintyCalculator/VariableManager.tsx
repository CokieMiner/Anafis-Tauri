...
const LIST_CONTAINER_STYLES = {
  ...
  // Firefox
  scrollbarWidth: 'thin',
  scrollbarColor: (theme: Theme) => `${theme.palette.primary.main} ${theme.palette.background.default}`,
...
interface VariableManagerProps {
  ...
  onVariableUpdate: (index: number, field: keyof Variable, value: string) => void;
}
...
const VariableManager: React.FC<VariableManagerProps> = memo(({
  ...
  const handleUncertaintyChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
          ...
          {variables.map((variable, index) => (
            <ListItemButton
              ...
              sx={{
                ...
                bgcolor: selectedIndex === index ? 'rgba(156, 39, 176, 0.1)' : 'transparent',
                '&:hover': {
                  bgcolor: selectedIndex === index ? 'rgba(156, 39, 176, 0.15)' : 'rgba(255,255,255,0.05)',
