...
/// Import Parquet file
pub async fn import_parquet(file_path: &str) -> Result<ImportResponse, String> {
...
fn convert_parquet_row_to_json(row: &Row, num_columns: usize) -> Result<Vec<Value>, String> {
    ...
    for i in 0..num_columns {
        ...
        let value = match field.1 {
            ...
            parquet::record::Field::TimestampMillis(ts) => Value::Number((*ts).into()),
            parquet::record::Field::TimestampMicros(ts) => Value::Number((*ts).into()),
            parquet::record::Field::TimeMillis(t) => Value::Number((*t as i64).into()),
            parquet::record::Field::TimeMicros(t) => Value::Number((*t).into()),
            parquet::record::Field::Date(d) => Value::Number((*d as i64).into()),
