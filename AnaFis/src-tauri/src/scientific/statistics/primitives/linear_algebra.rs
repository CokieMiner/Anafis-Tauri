...
impl LinearAlgebra {
    ...
    /// Solve least squares problem min ||Ax - b|| using SVD
    ///
    /// This is more numerically stable than normal equations for ill-conditioned matrices.
    /// Uses SVD decomposition to solve Ax ≈ b in a regularized way.
    pub fn least_squares(a: &Array2<f64>, b: &Array1<f64>) -> Result<Array1<f64>, String> {
        let (u, s, vt) = Self::svd(a)?;

        // Compute pseudo-inverse using SVD
        // For least squares: x = V * S^+ * U^T * b
        // where S^+ is the pseudo-inverse of the diagonal matrix S

        let m = a.nrows();
        let n = a.ncols();
        let min_dim = m.min(n);

        // Threshold for singular values (relative to largest singular value)
        let threshold = s[0] * 1e-12; // Machine precision threshold

        // Compute S^+ (pseudo-inverse of singular values)
        let mut s_inv = Array1::zeros(min_dim);
        for i in 0..min_dim {
            if s[i] > threshold {
                s_inv[i] = 1.0 / s[i];
            }
        ...
        // Compute S^+ * (U^T * b)
        let s_inv_u_t_b = &s_inv * &u_t_b;

        // Compute V * (S^+ * U^T * b)
        let x = vt.t().dot(&s_inv_u_t_b);
    ...
    pub fn covariance_matrix(data: &Array2<f64>) -> Result<Array2<f64>, String> {
        Self::covariance_matrix_with_ddof(data, 1.0)
    }

    /// Covariance matrix computation with configurable degrees of freedom
    pub fn covariance_matrix_with_ddof(data: &Array2<f64>, ddof: f64) -> Result<Array2<f64>, String> {
        if data.nrows() < 2 {
        ...
        let cov = centered.t().dot(&centered) / (n - ddof);
        Ok(cov)
    }
