...
//!
//! This module provides comprehensive uncertainty propagation techniques
//! for propagating measurement uncertainties through mathematical functions.

use ndarray::Array2;
use num_dual::Dual64;

/// Uncertainty propagation engine for measurement errors
...
impl UncertaintyPropagationEngine {
    ...
    pub fn covariance_matrix_from_uncertainties(
        ...
        UncertaintyPropagation::covariance_matrix_from_uncertainties(uncertainties, correlations)
    }
    ...
    ///
    /// This implementation uses Dual Numbers for automatic differentiation to compute exact derivatives.
    ...
    where
        F: Fn(&[Dual64]) -> Dual64,
    {
        UncertaintyPropagation::propagate_uncertainty(function, variables, covariance_matrix)
    }
    ...
    pub fn propagate_mean_uncertainty(
        ...
        UncertaintyPropagation::propagate_mean_uncertainty(values, uncertainties, correlations)
    }
    ...
    pub fn propagate_variance_uncertainty(
        ...
        UncertaintyPropagation::propagate_variance_uncertainty(values, uncertainties, correlations)
    }
    ...
    pub fn propagate_correlation_uncertainty(
        ...
        UncertaintyPropagation::propagate_correlation_uncertainty(x_values, y_values, x_uncertainties, y_uncertainties, xy_correlations)
    }

    /// Simplified uncertainty propagation for common functions
    /// For functions where analytical derivatives are known
    pub fn propagate_simple_function_uncertainty<F, G>(
        function: F,
        derivative: G,
        variables: &[f64],
        uncertainties: &[f64],
    ...
    where
        F: Fn(&[f64]) -> f64,
        G: Fn(&[f64]) -> Vec<f64>, // Returns partial derivatives
    {
        UncertaintyPropagation::propagate_simple_function_uncertainty(function, derivative, variables, uncertainties)
    }
...
impl UncertaintyPropagation {
    ...
    pub fn covariance_matrix_from_uncertainties(
        ...
        if n == 0 {
            return Err("Cannot create covariance matrix from empty uncertainties".to_string());
        }
        ...
        F: Fn(&[Dual64]) -> Dual64,
    {
        ...
        // Calculate partial derivatives using Dual Numbers
        for (i, partial) in partials.iter_mut().enumerate() {
            let mut dual_vars = Vec::with_capacity(n);
            for (j, &val) in variables.iter().enumerate() {
                if i == j {
                    dual_vars.push(Dual64::new(val, 1.0)); // Variable of interest
                } else {
                    dual_vars.push(Dual64::new(val, 0.0)); // Constant
                }
            }
            
            let result = function(&dual_vars);
            *partial = result.eps; // The derivative part
        }
    ...
    where
        ...
        G: Fn(&[f64]) -> Vec<f64>, // Returns partial derivatives
    {
        if variables.len() != uncertainties.len() {
            return Err("Variables and uncertainties must have the same length".to_string());
        }

        let partials = derivative(variables);

        // For uncorrelated variables: σ_f² = Σᵢ (∂f/∂xᵢ)² σ_{xᵢ}²
        let mut variance = 0.0;
        for i in 0..variables.len() {
            variance += partials[i].powi(2) * uncertainties[i].powi(2);
        }
