...
WHITELISTED_ALIAS_IMPORT_PREFIXES = {
    "src/lib.rs": {
        ("data_commands", "src::data_library::commands"),
        ("curve_commands", "src::scientific::curve_fitting::commands"),
        ("math_commands", "src::scientific::math_functions"),
        ("uncertainty_calc", "src::scientific::uncertainty_propagation::calculator"),
        ("unit_commands", "src::unit_conversion::commands"),
        ("file_ops", "src::utils::file_operations"),
        ("window_commands", "src::windows::secondary_windows"),
        ("manager_commands", "src::windows::window_manager"),
    }
}
...
def staircase_boundaries() -> set[str]:
    ...
    for directory in SRC.rglob("logic"):
        if directory.is_dir():
            rel = directory.parent.relative_to(ROOT)
...
def qualified_path_matches(line: str, path: str, local_aliases: set[str] | None = None) -> list[str]:
    matches: list[str] = []
    for match in QUALIFIED_PATH_RE.findall(line):
        candidate = match.strip()
        if candidate.startswith("$crate::") or candidate.startswith("crate::"):
            continue
        ...
        if any(candidate.startswith(prefix) for prefix in WHITELISTED_QUALIFIED_PREFIXES):
            continue
        if path in WHITELISTED_PATH_PREFIXES:
            if any(candidate.startswith(p) for p in WHITELISTED_PATH_PREFIXES[path]):
                continue
    ...
    for path in sorted(SRC.rglob("*.rs")):
        ...
        for lineno, line in enumerate(lines, start=1):
            ...
            if not inside_test_module and not inside_macro:
                rel_path = str(path.relative_to(ROOT))
                local_aliases = {item.alias for item in alias_imports if item.path == rel_path and item.alias}
                for match in qualified_path_matches(line, rel_path, local_aliases):
                    finding_keys.add(
                        (
